...
- **Sudoku.Tests** (net10.0) — NUnit 4 test project using FluentAssertions. Test data files (`sudokus.json`, `sudokus.txt`) are copied to output on build.
- **Sudoku.Benchmarks** (net10.0) — BenchmarkDotNet performance benchmarks for solver implementations.
...
- **Koan content invariants.** Curated content lives in `Sudoku.App/Resources/Raw/koans.json` (single source of truth — `Sudoku.Tests` consumes it via `<Link>` in `Sudoku.Tests.csproj`). The active pool is filtered at load time by `KoanParser.FilterPlayablePool` (currently `tone != "sharp"`); changing the filter requires updating `s_knownTones` and the threshold in `KoansFile_PlayablePoolIsNotEmpty` in `KoansAssetTests`. `MenuViewModel.NewGame` branches on `Settings.ShowKoans` to navigate to `KoanPage` or directly to `GamePage`; `ContinueGame` (saved-game resume) does not branch and never goes through the koan path. IDs in the JSON are author metadata only — gaps are fine (id 107 is intentionally absent, was a duplicate of id 57).
